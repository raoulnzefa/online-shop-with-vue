      ...
      <div class="form-group">
        ...
        <select
          ...
          <template v-for="manufacturer in manufacturers">
            <option :value="manufacturer._id" :selected="manufacturer._id == (model.manufacturer && model.manufacturer._id)" add v-bind:key="manufacturer.id">{{manufacturer.name}}</option>
          </template>
        </select>
...
export default {
  ...
  data: function data () {
    return {
      model: { },
      manufacturers: [{ _id: '0', name: 'Samsung' }, { _id: '1', name: 'Apple' }]
